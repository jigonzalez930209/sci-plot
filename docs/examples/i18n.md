...
Sci Plot supports multiple locales for number formatting, date/time display, and UI labels. Select a language to see how formatting changes across all elements.
...
```typescript
import { setGlobalLocale, getGlobalLocale } from 'sci-plot';
...
```typescript
import { registerLocale } from 'sci-plot';
